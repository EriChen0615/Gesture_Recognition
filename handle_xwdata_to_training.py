...
if __name__ == '__main__':
   ...
   # --------- CONFIGURATION STARTS --------- #
    ...
    GS_BBOX_DICT = {'one':(WINDOW_WIDTH*SHORT_FAC,WINDOW_HEIGHT*LONG_FAC),
                    ...
                    'others':(WINDOW_WIDTH*SHORT_FAC,WINDOW_HEIGHT*LONG_FAC)}

    COLLECT_LABEL = ['one','fist','others'] # the label to collect

    # DATA PATH
    DIR_NAME = os.path.join('XW-Dataset','Training')
    ...
    with open(os.path.join('XW-Dataset','Training','imglist_with_gesture.txt'),'w+') as f_out:
        for gs,f_gs in gest_anno.items():
            ...
            for anno in annos:
                ...
                if gs == 'one':
                    ...
                    append_field(words,OTHERS_FIELD)
                elif gs == 'fist':
