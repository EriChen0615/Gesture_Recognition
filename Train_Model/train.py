...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    if with_gesture:
        cls_loss_op,bbox_loss_op,gesture_loss_op,L2_loss_op,accuracy_op = net_factory(input_image, label, bbox_target,gesture_target,training=True,with_gesture=with_gesture)
        total_loss_op  = radio_cls_loss*cls_loss_op + radio_bbox_loss*bbox_loss_op + radio_gesture_loss*gesture_loss_op + L2_loss_op
    else:
        cls_loss_op,bbox_loss_op, L2_loss_op,accuracy_op = net_factory(input_image, label, bbox_target,gesture_target, training=True,with_gesture=with_gesture)
