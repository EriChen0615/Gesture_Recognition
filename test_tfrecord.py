...
def read():
    ...
    with tf.Session() as sess:
        ...
        try:
            while not coord.should_stop() and i < 1:
                #在这里之使用一个batch
                im_batch, label_batch, rois_batch, gesture_batch = sess.run([images, labels,rois,gestures])
                i+=1
    ...
    for i in range(num_batch):
        ...
        #print roi_batch
        #print gesture_batch
        print('label for this image: {0}. Gesture: {1}'.format(label_batch[i],gesture_batch[i]))
        print('bbox:{0}'.format(rois_batch[i]))
        print('===================')
        #for j in range(5):
...
if __name__ == '__main__':
    # BATCH_SIZE = 64
    # net = 'PNet'
    # dataset_dir = "Dataset/Training/imglists/PNet"
    # gesture_dir = os.path.join(dataset_dir,'train_PNet_gesture.tfrecord_shuffle')
    # images, labels, rois,gestures  = read_single_tfrecord(gesture_dir, BATCH_SIZE, net)
    # print('Shapes: img {0}, labels {1}, rois {2}, gesture{3}'.format(images.shape,labels.shape,rois.shape,gestures.shape))
    # print(gestures[-1])
    read()
